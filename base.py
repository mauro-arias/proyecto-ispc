...
for fila in lista_personas:
	...
	try:
		...
		pais = session_mysql.query(Pais).filter(Pais.nombre == fila['country']).first()
		if pais == None :
			pais = Pais(nombre=fila['country'])
			session_mysql.add(pais)

		ciudad = session_mysql.query(Ciudad).filter(Ciudad.nombre == fila['city']).first()
		if ciudad == None:
			ciudad = Ciudad(nombre=fila['city'])
			session_mysql.add(ciudad)

		barrio = session_mysql.query(Barrio).filter(Barrio.nombre == fila['town']).first()
		if barrio == None:
			barrio = Barrio(nombre=fila['town'])
			session_mysql.add(barrio)

		provincia = session_mysql.query(Provincia).filter(Provincia.nombre == fila['state']).first()
		if provincia == None:
			provincia = Provincia(nombre=fila['state'])
			session_mysql.add(provincia)

		lugar = session_mysql.query(Lugar).filter(and_ (Lugar.pais == pais , Lugar.barrio == barrio , Lugar.provincia == provincia , Lugar.ciudad == ciudad)).first()
		if lugar == None:
			lugar = Lugar(pais=pais, barrio=barrio, provincia=provincia, ciudad=ciudad)
			session_mysql.add(lugar)
		# ************************************************************************************************

		genero = session_mysql.query(Genero).filter(Genero.nombre == fila['gender']).first()
		if genero == None :
			genero = Genero(nombre=fila['gender'])
			session_mysql.add(genero)

		tipopersona = session_mysql.query(TipoPersona).filter(TipoPersona.nombre == fila['tipopersona']).first()
		if tipopersona == None :
		...
		persona = session_mysql.query(Persona).filter(Persona.personal_id == fila['personal_id']).first()
		if persona == None :
...
###
persona=session_mysql.query(Persona).join(TipoPersona).join(Genero).filter(and_ (TipoPersona.nombre=="Alumno",Genero.nombre=="Female" )).first()

persona.__dict__

tipo=session_mysql.query(TipoPersona).filter(TipoPersona.nombre=='Alumno').first()
alumnos=session_mysql.query(Persona).join(TipoPersona).join(Genero).filter(and_(Persona.age<40, Persona.tipopersona==tipo)).all()

for cadaUno in alumnos:
    print(cadaUno.nombre, cadaUno.apellido, cadaUno.email, cadaUno.genero.nombre, cadaUno.birthdate, cadaUno.lugar.pais.nombre,cadaUno.personal_id, cadaUno.lugar.ciudad.nombre,cadaUno.lugar.barrio.nombre, cadaUno.lugar.provincia.nombre, cadaUno.tipopersona.nombre,cadaUno.age)
  

todos=session_mysql.query(Persona).join(TipoPersona).join(Genero).filter(Persona.age>40).all()
